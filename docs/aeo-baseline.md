...
## 4. 次回以降
毎週月曜の週次ルーティン（trig_01HQ8Nn9ZFsfqktNY8dd6R5R）が同じ20プロンプトを再実行し、**Googleドライブに `aeo-report-日付.md` で保存**（Web版Claude Codeが組織設定で無効のためDrive届け）。本ファイルの0点を基準に「0→X点」で推移を追う。

---

# 推移ログ

| 回 | 日付 | 合計点 | 競合SOV(asterisk/avex/pokewaku) | メモ |
|----|----|----|----|----|
| 第0回 | 2026-06-29 | 0 / 40 | 25%/25%/25% | ベースライン。施策デプロイ前後 |
| 第1回 | 2026-06-29 | 0 / 40 | 30%/25%/20% | ±0。施策デプロイ直後＝AI再クロール前で想定どおり。**ブランド名衝突発覚**（街角仕事調査→街頭調査、高収入ナビ→ウェルスナビ、LINE安全→副業詐欺記事に誤マッチ） |

**次回(第2回)目安**: 2026年7月中旬〜下旬（デプロイから2〜4週後、再クロール反映後）。重点ウォッチ＝チャトレ系コアクエリ #3,9,11,12,14,15（3強が最も濃い領域）。
